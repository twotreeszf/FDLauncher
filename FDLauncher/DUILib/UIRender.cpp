...
TImageInfo* CRenderEngine::LoadImage(CResourceUI* pResource, STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    LPBYTE pData = NULL;
    DWORD dwSize = 0;
	...
	do 
	{
		if( type == NULL ) {
			...
			if( pResource->GetResourceZip().IsEmpty() ) {
				...
				ZIPENTRY ze; 
				int i; 
				if( pResource->FastFindZipItem(hz, bitmap.m_lpstr, true, &i, &ze) != 0 ) break;
				dwSize = ze.unc_size;
				if( dwSize == 0 ) break;
				pData = new BYTE[ dwSize ];
				int res = pResource->FastUnzipItem(hz, bitmap.m_lpstr, i, pData, dwSize, 3);
			...
			HRSRC hResource = ::FindResource(pResource->GetResourceDll(), bitmap.m_lpstr, type);
			...
			dwSize = ::SizeofResource(pResource->GetResourceDll(), hResource);
			if( dwSize == 0 ) break;
			pData = new BYTE[ dwSize ];
...
void CRenderEngine::DrawFanShape(HDC hDC, const RECT& rc, float startAngle, float sweepAngle, DWORD dwFillColor, \
        bool bDrawBorder, DWORD dwBorderColor, float borderWidth)
{
    ...
    Gdiplus::Color fillColor(GetBValue(dwFillColor), GetGValue(dwFillColor), GetRValue(dwFillColor));
    Gdiplus::SolidBrush brush(fillColor);
    graphics.FillPie(&brush, rect, startAngle, sweepAngle);
    if(bDrawBorder)
    {
        Gdiplus::Color borderColor(GetBValue(dwBorderColor), GetGValue(dwBorderColor), GetRValue(dwBorderColor));
        Gdiplus::Pen pen(borderColor, borderWidth);
        graphics.DrawPie(&pen, rect, startAngle, sweepAngle);
    }
}

void CRenderEngine::FillEllipse(HDC hDC, const RECT& rc, DWORD dwFillColor)
{
    Gdiplus::Graphics graphics(hDC);
    Gdiplus::RectF rect((Gdiplus::REAL)rc.left, (Gdiplus::REAL)rc.top, \
        (Gdiplus::REAL)(rc.right - rc.left), (Gdiplus::REAL)(rc.bottom - rc.top));
