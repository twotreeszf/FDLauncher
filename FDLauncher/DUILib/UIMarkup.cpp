...
bool CMarkup::LoadFromFile(CResourceUI* pResource, LPCTSTR pstrFilename, int encoding)
{
    Release();
    ...
    if( pResource->GetResourceZip().IsEmpty() ) {
        ...
        ZIPENTRY ze; 
        int i; 
        if( pResource->FastFindZipItem(hz, pstrFilename, true, &i, &ze) != 0 ) return _Failed(_T("Could not find ziped file"));
        DWORD dwSize = ze.unc_size;
        if( dwSize == 0 ) return _Failed(_T("File is empty"));
        if ( dwSize > 4096*1024 ) return _Failed(_T("File too large"));
        BYTE* pByte = new BYTE[ dwSize ];
        int res = pResource->FastUnzipItem(hz, pstrFilename, i, pByte, dwSize, 3);
