...
CListUI::CListUI(CContainerUI* pListBody) : m_pCallback(NULL), m_bScrollSelect(false), m_bMultiSelect(false), m_bOwnerSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
	if (!pListBody)
...
bool CListUI::AddAt(CControlUI* pControl, int iIndex)
{
	...
	for (int i = 0;i < m_aCurSelItems.GetSize();i++)
	{
		int index = (int)m_aCurSelItems[i];
		if(index >= iIndex)
		{
			m_aCurSelItems.SetAt(i, (LPVOID)(index + 1));
		}
	}
    return true;
}
...
bool CListUI::Remove(CControlUI* pControl)
{
	...
	for (int i = 0;i < m_aCurSelItems.GetSize();)
	{
		if((int)m_aCurSelItems[i] == iIndex)
		{
			m_aCurSelItems.Remove(i);			
		}
		else if((int)m_aCurSelItems[i] > iIndex)
		{
			m_aCurSelItems.SetAt(i, (LPVOID)((int)(m_aCurSelItems[i]) - 1));
			i++;
		}
		else
		{
    ...
    if( iIndex == m_iCurSel && m_iCurSel >= 0 ) {
        int iSel = m_iCurSel;
        m_iCurSel = -1;
        SelectItem(FindSelectable(iSel, false), true);
    }
...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_KEYDOWN:
        ...
        case VK_RETURN:
            ...
            return;
		case L'A' :
			if(m_bMultiSelect && (event.wKeyState & MK_CONTROL) != 0){
				SelectAll();
			}
			break;
            }
...
{
	return m_aCurSelItems.GetSize();
}
int CListUI::GetSelItemIndex(int i) const
{
	return (int)m_aCurSelItems[i];
}

bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    if( iIndex == m_iCurSel && m_aCurSelItems.GetSize() == 1 ) return true;
	if(m_bOwnerSelect) return true;

	CBoolReseter reset(m_bOwnerSelect);
	...
	if(multiItem && iIndex >= 0) // 多选
	{	
		bool bSelect = true;
		if(controlDown)
		{			
			int i = m_aCurSelItems.Find((LPVOID)iIndex);
			if(i >= 0)
			{
				bSelect = false;
			}			
		}
		return MultiSelectItem(iIndex, bSelect, bTakeFocus);
	}
	else if(multiRange && iIndex >= 0 && m_iCurSel >= 0) // 多选区域
	{		
		return MultiSelectRange(iIndex, true, bTakeFocus);
	}
	else
	{
		return SingleSelect(iIndex, bTakeFocus);		
	}

    return true;
}

bool CListUI::SelectAll()
{
	CBoolReseter reset(m_bOwnerSelect);
	if(m_bMultiSelect)
	{
		...
		for (int i = 0;i < GetCount();i++)
		{
			...
			if( pListItem->Select(true) ) {
				m_aCurSelItems.Add((LPVOID)i);
			}
		}
		if(m_aCurSelItems.GetSize() > 0)
		{
			if(m_iCurSel < 0) 
				m_iCurSel = (int)m_aCurSelItems[0];
		}
		else
		{
			m_iCurSel = -1;
		...
		if( m_pManager != NULL ) {
			m_pManager->SendNotify(this, _T("itemselect"), m_iCurSel, -1);
		}
...
bool CListUI::SingleSelect(int iIndex, bool bTakeFocus, bool bEnsureVisible /*= true*/)
{	
	int iOldSel = m_iCurSel;
	// We should first unselect the currently selected item
	if( m_iCurSel >= 0) {
		ASSERT(m_aCurSelItems.GetSize() > 0);
		for (int i = 0;i < m_aCurSelItems.GetSize();i++)
		{
			int item = (int)m_aCurSelItems[i];
			CControlUI* pControl = GetItemAt(item);
			if( pControl != NULL) {
				IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
	...
	m_iCurSel = iIndex;
	m_aCurSelItems.Add((LPVOID)m_iCurSel);
	if( !pListItem->Select(true) ) {
		m_iCurSel = -1;
		m_aCurSelItems.Empty();
		return false;
...
bool CListUI::MultiSelectItem(int iIndex, bool bSelect, bool bTakeFocus, bool bEnsureVisible/* = true*/)
{
	if(bSelect)
	{
		CControlUI* pControl = GetItemAt(iIndex);
		...
		m_iCurSel = iIndex;
		if(m_aCurSelItems.Find((LPVOID)iIndex) < 0)
		{
			m_aCurSelItems.Add((LPVOID)m_iCurSel);
		}		
		if( !pListItem->Select(true) ) {			
			m_aCurSelItems.Remove(m_aCurSelItems.GetSize() - 1);
			if(m_aCurSelItems.GetSize() > 0)
			{
				m_iCurSel = (int)m_aCurSelItems[m_aCurSelItems.GetSize() - 1];
			}
			else
			{
				m_iCurSel = -1;
	...
	{
		UnselectItem(iIndex, bTakeFocus);
	}
	
	return true;
}

bool CListUI::MultiSelectRange(int iIndex, bool bSelect, bool bTakeFocus)
{	
	if(bSelect)
	{
		CControlUI* pControl = GetItemAt(iIndex);
		...
		int nFirst = min(iIndex, m_iCurSel);
		int nLast  = max(iIndex, m_iCurSel);
		...
		for (int i = 0;i < m_aCurSelItems.GetSize();i++)
		{
			selItems.push_back((int)m_aCurSelItems[i]);
		}
		std::sort(selItems.begin(), selItems.end());
		for (int i = 0;i < (int)selItems.size() - 1;i++)
		{
			if(selItems[i + 1] - selItems[i] > Diff)
			{
				Diff = selItems[i + 1] - selItems[i];
			}
		}
		...
		else if(!selItems.empty())
		{
			nFirst = min(selItems[0], nFirst);
			nLast  = max(selItems[selItems.size() - 1], nLast);
		}		
		m_aCurSelItems.Empty();
		...
		m_iCurSel = nLastIndex;
		if(nLastIndex >= 0)
		{
			CControlUI* pControl = GetItemAt(nLastIndex);	
			if(pControl)
			{
				if( bTakeFocus ) pControl->SetFocus();
				if( m_pManager != NULL ) {
...
void CListUI::UnselectItem( int iIndex, bool bTakeFocus /*= false*/)
{
	CControlUI* pControl = GetItemAt(iIndex);
	if( pControl == NULL ) return;
	if( !pControl->IsVisible() ) return;
	if( !pControl->IsEnabled() ) return;
	...
	else if(m_aCurSelItems.GetSize() == 0)
	{
		m_iCurSel = -1;
		if( m_pManager != NULL ) {
			m_pManager->SendNotify(this, _T("itemselect"), m_iCurSel, iOldSel);
...
void CListUI::SetScrollLength(int iScrollLenth)
{
    if (m_ListInfo.iScrollLenth == iScrollLenth) return;
    m_ListInfo.iScrollLenth = iScrollLenth;
    m_pList->SetScrollLenth(iScrollLenth);
}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcscmp(pstrName, _T("scrolllength")) == 0) SetScrollLength(_ttoi(pstrValue));
...
void CListBodyUI::SetPos(RECT rc)
{
    ...
    // EnsureVisible
    m_pOwner->EnsureVisible(m_pOwner->GetCurSel());
}
...
bool CListElementUI::IsHot() const
{
...
CListTextElementUI::CListTextElementUI() : m_nLinks(0), m_nHoverLink(-1), m_pOwner(NULL), m_nHoverColumn(-1)
{
...
CListTextElementUI::~CListTextElementUI()
{
	...
	for( int it = 0; it < m_aTooltips.GetSize(); it++ ) {
		pText = static_cast<CStdString*>(m_aTooltips[it]);
		if( pText ) delete pText;
	}
	m_aTooltips.Empty();
}
...
CStdString CListTextElementUI::GetToolTip() const
{
	if(m_nHoverColumn >= 0 && m_nHoverColumn < m_aTooltips.GetSize())
	{
		CStdString* pText = static_cast<CStdString*>(m_aTooltips[m_nHoverColumn]); 
		if (pText)
		{
			return *pText;
		}
	}
	return CListLabelElementUI::GetToolTip();
}

void CListTextElementUI::SetToolTip(LPCTSTR pstrText)
{
	CListLabelElementUI::SetToolTip(pstrText);
}

CStdString CListTextElementUI::GetToolTip(int iIndex) const
{
	ASSERT(iIndex >= 0 && iIndex < m_aTooltips.GetSize());
	
	CStdString* pText = static_cast<CStdString*>(m_aTooltips.GetAt(iIndex));
	if( pText ) return *pText;
	return CStdString();
}

void CListTextElementUI::SetToolTip(int iIndex, LPCTSTR pstrText)
{
	if( m_pOwner == NULL ) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	if( iIndex < 0 || iIndex >= pInfo->nColumns ) return;
	while( m_aTooltips.GetSize() < pInfo->nColumns ) { m_aTooltips.Add(NULL); }

	CStdString* pText = static_cast<CStdString*>(m_aTooltips[iIndex]);
	if( (pText == NULL && pstrText == NULL) || (pText && *pText == pstrText) ) return;

	if( pText ) delete pText;
	m_aTooltips.SetAt(iIndex, new CStdString(pstrText));
}
...
void CListTextElementUI::DoEvent(TEventUI& event)
{
    ...
    if( m_nLinks > 0 && event.Type == UIEVENT_MOUSEMOVE ) {
		...
		if(m_aTooltips.GetSize() > 0)
		{			
			int nHover = GetHoverColumn(event.ptMouse);
			// 这里让tooltip失效，否则一直显示在原来位置
			if(m_nHoverColumn != nHover)
			{				
				TOOLINFO toolTip;
				toolTip.cbSize = sizeof(TOOLINFO);
				if(GetManager()->GetTooltipWindow() != NULL)
				{
					::SendMessage(GetManager()->GetTooltipWindow(), TTM_GETTOOLINFO, 0, (LPARAM)&toolTip); 
					::SendMessage(GetManager()->GetTooltipWindow(), TTM_TRACKACTIVATE, FALSE, (LPARAM) &toolTip);
				}			
			}
		}
    }
	...
	if(m_pOwner != NULL && m_aTooltips.GetSize() > 0 &&event.Type == UIEVENT_MOUSEHOVER)
	{		
		int nHover = GetHoverColumn(event.ptMouse);
		m_nHoverColumn = nHover;
	}
...
int CListTextElementUI::GetHoverColumn(POINT pt)
{
	...
	CRect rc = GetPos();
	for (int i = 0;i < pInfo->nColumns; i++){			
		rc.left = pInfo->rcColumn[i].left;
		rc.right = pInfo->rcColumn[i].right;			
		if(::PtInRect(&rc ,pt)){
			nHover = i;
			break;
		}
...
CListGroupElementUI::CListGroupElementUI(int nColumn, int pInnerRow[])
    : m_nColumn(nColumn)
    , m_iHoverColumn(-1)
    , m_iHoverRow(-1)
    , m_nLinks(0)
    ...
    , m_uButtonState(0)
{
    ASSERT(nColumn > 0 && pInnerRow);

    m_pInnerRow = new int[m_nColumn];
    m_pCellText = new CStdString*[m_nColumn];
    m_pToolTip  = new CStdString*[m_nColumn];
    m_pCellRect = new RECT*[m_nColumn];
    for(int i = 0; i < m_nColumn; ++i)
    {
        m_pCellText[i]  = new CStdString[pInnerRow[i]];
        m_pToolTip[i]   = new CStdString[pInnerRow[i]];
        m_pCellRect[i]  = new RECT[pInnerRow[i]];
        m_pInnerRow[i]  = pInnerRow[i];

        if(m_nMaxLinks < pInnerRow[i]) m_nMaxLinks = pInnerRow[i];
    }

    m_nMaxLinks *= MAX_LINK;
    m_pRcLinks = new RECT[m_nMaxLinks];
    m_pSzLinks = new CStdString[m_nMaxLinks];
}

CListGroupElementUI::~CListGroupElementUI()
{
    for(int i = 0; i < m_nColumn; ++i)
    {
        delete []m_pCellText[i];
        delete []m_pToolTip[i];
        delete []m_pCellRect[i];
    }
...
LPCTSTR CListGroupElementUI::GetClass() const
{
    return _T("ListGroupElementUI");
}

LPVOID CListGroupElementUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcscmp(pstrName, _T("ListItem")) == 0 ) return static_cast<IListItemUI*>(this);
    if( _tcscmp(pstrName, _T("ListGroupElement")) == 0 ) return static_cast<CListGroupElementUI*>(this);
    return CControlUI::GetInterface(pstrName);
}

UINT CListGroupElementUI::GetControlFlags() const
{
    return UIFLAG_WANTRETURN | ((IsEnabled() && m_nLinks > 0) ? UIFLAG_SETCURSOR : 0);
}

void CListGroupElementUI::SetPos(RECT rc)
{
    CControlUI::SetPos(rc);

    TListInfoUI* pInfo = (m_pOwner ? m_pOwner->GetListInfo() : NULL);
    if(pInfo && m_nColumn <= pInfo->nColumns)
    {
        for(int i = 0; i < m_nColumn; ++i)
        {
            LONG lSpace = (m_rcItem.bottom - m_rcItem.top) / m_pInnerRow[i];
            RECT rcCell = {pInfo->rcColumn[i].left, m_rcItem.top, pInfo->rcColumn[i].right, m_rcItem.top + lSpace};
            for(int j = 0; j < m_pInnerRow[i]; ++j)
            {
                m_pCellRect[i][j] = rcCell;
                rcCell.top    += lSpace;
                rcCell.bottom += lSpace;
            }
        }
    }
}

LPCTSTR CListGroupElementUI::GetCellText(int iColumnIndex, int iInnerRowIndex) const
{
    LPCTSTR szText = NULL;
    if(iColumnIndex >= 0 && iColumnIndex < m_nColumn 
        && iInnerRowIndex >= 0 && iInnerRowIndex < m_pInnerRow[iColumnIndex])
    {
        szText = m_pCellText[iColumnIndex][iInnerRowIndex].GetData();
    }

    return szText;
}

void CListGroupElementUI::SetCellText(int iColumnIndex, int iInnerRowIndex, LPCTSTR pstrText)
{
    if(pstrText && iColumnIndex >= 0 && iColumnIndex < m_nColumn 
        && iInnerRowIndex >= 0 && iInnerRowIndex < m_pInnerRow[iColumnIndex])
    {
        if(m_pCellText[iColumnIndex][iInnerRowIndex] != pstrText)
        {
            m_pCellText[iColumnIndex][iInnerRowIndex] = pstrText;
            Invalidate();
        }
    }
}

LPCTSTR CListGroupElementUI::GetToolTip(int iColumnIndex, int iInnerRowIndex) const
{
    LPCTSTR szText = NULL;
    if(iColumnIndex >= 0 && iColumnIndex < m_nColumn 
        && iInnerRowIndex >= 0 && iInnerRowIndex < m_pInnerRow[iColumnIndex])
    {
        szText = m_pToolTip[iColumnIndex][iInnerRowIndex].GetData();
    }

    return szText;
}

void CListGroupElementUI::SetToolTip(int iColumnIndex, int iInnerRowIndex, LPCTSTR pstrText)
{
    if(pstrText && iColumnIndex >= 0 && iColumnIndex < m_nColumn 
        && iInnerRowIndex >= 0 && iInnerRowIndex < m_pInnerRow[iColumnIndex])
    {
        if(m_pToolTip[iColumnIndex][iInnerRowIndex] != pstrText)
        {
            m_pToolTip[iColumnIndex][iInnerRowIndex] = pstrText;
        }
    }
}

CStdString CListGroupElementUI::GetToolTip() const
{
    return CControlUI::GetToolTip().GetData();
}

void CListGroupElementUI::SetToolTip(LPCTSTR pstrText)
{
    CControlUI::SetToolTip(pstrText);
}

void CListGroupElementUI::GetHoverCell(int& iHoverColumn, int& iHoverRow)
{
    iHoverColumn = m_iHoverColumn;
    iHoverRow    = m_iHoverRow;
}

int CListGroupElementUI::GetIndex() const
{
    return m_iIndex;
}

void CListGroupElementUI::SetIndex(int iIndex)
{
    m_iIndex = iIndex;
}

IListOwnerUI* CListGroupElementUI::GetOwner()
{
    return m_pOwner;
}

void CListGroupElementUI::SetOwner(CControlUI* pOwner)
{
    m_pOwner = static_cast<IListUI*>(pOwner->GetInterface(_T("IList")));
    ASSERT(m_nColumn <= m_pOwner->GetListInfo()->nColumns);
}

bool CListGroupElementUI::IsSelected() const
{
    return m_bSelectable && m_bSelected;
}

bool CListGroupElementUI::Select(bool bSelect)
{
    if(!IsEnabled() || !m_bSelectable) return false;
    if(bSelect == m_bSelected) return true;
    m_bSelected = bSelect;
...
bool CListGroupElementUI::IsExpanded() const
{
    return false;
}

bool CListGroupElementUI::Expand(bool bExpand)
{
    return false;
}

bool CListGroupElementUI::IsHot() const
{
	return (m_uButtonState & UISTATE_HOT) != 0;
}

void CListGroupElementUI::DoEvent(TEventUI& aEvent)
{
    if(!IsMouseEnabled() && aEvent.Type > UIEVENT__MOUSEBEGIN && aEvent.Type < UIEVENT__MOUSEEND)
    {
        if(m_pOwner)
        {
            m_pOwner->DoEvent(aEvent);
        }
        else
        {
            if(aEvent.Type == UIEVENT_MOUSEENTER)
            {
                if(IsEnabled())
                {
                    m_uButtonState |= UISTATE_HOT;
            ...
            if(aEvent.Type == UIEVENT_MOUSELEAVE)
            {
                if((m_uButtonState & UISTATE_HOT) != 0)
                {
                    m_uButtonState &= ~UISTATE_HOT;
    ...
    // When you hover over a link
    if(aEvent.Type == UIEVENT_SETCURSOR)
    {
        for(int i = 0; i < m_nLinks; i++)
        {
            if(::PtInRect((m_pRcLinks + i), aEvent.ptMouse))
            {
                ::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE(IDC_HAND)));
    ...
    if(aEvent.Type == UIEVENT_MOUSEHOVER)
    {
        for(int i = 0; i < m_nColumn; ++i)
        {
            for(int j = 0; j < m_pInnerRow[i]; ++j)
            {
                if(::PtInRect(&m_pCellRect[i][j], aEvent.ptMouse))
                {
                    SetToolTip(m_pToolTip[i][j]);;
    ...
    if(aEvent.Type == UIEVENT_BUTTONUP && IsEnabled())
    {
        for(int i = 0; i < m_nLinks; i++)
        {
            if(::PtInRect((m_pRcLinks + i), aEvent.ptMouse))
    ...
    if(aEvent.Type == UIEVENT_MOUSEMOVE)
    {
        bool bFlag = false;
        if(m_nLinks > 0)
        {
        ...
        for(int i = 0; i < m_nColumn; ++i)
        {
            for(int j = 0; j < m_pInnerRow[i]; ++j)
            {
                if(::PtInRect(&m_pCellRect[i][j], aEvent.ptMouse))
                {
                    if(m_iHoverColumn != i || m_iHoverRow != j)
                    {
                        bFlag = true;
                        m_iHoverColumn = i;
                        m_iHoverRow    = j;
    ...
    if(aEvent.Type == UIEVENT_MOUSELEAVE)
    {
        ...
        if(m_iHoverColumn != -1 || m_iHoverRow != -1)
        {
            bFlag = true;
            m_iHoverColumn = -1;
            m_iHoverRow    = -1;
    ...
    if(m_pOwner) 
        ...
        CControlUI::DoEvent(aEvent);
}

SIZE CListGroupElementUI::EstimateSize(SIZE szAvailable)
{
    SIZE cXY = m_cxyFixed;
    TListInfoUI* pInfo = (m_pOwner ? m_pOwner->GetListInfo() : NULL);
...
void CListGroupElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
    if(!::IntersectRect(&m_rcPaint, &rcPaint, &m_rcItem)) return;
...
void CListGroupElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    if(!m_pOwner) return;
    ...
    m_nLinks   = 0;
    int nLinks = m_nMaxLinks;
    IListCallbackUI* pCallback = m_pOwner->GetTextCallback();
    for(int i = 0; i < m_nColumn; ++i)
    {
        for(int j = 0; j < m_pInnerRow[i]; ++j)
        {
            RECT rcItem     = m_pCellRect[i][j];
            rcItem.left     += pInfo->rcTextPadding.left;
            rcItem.right    -= pInfo->rcTextPadding.right;
            rcItem.top      += pInfo->rcTextPadding.top;
            rcItem.bottom   -= pInfo->rcTextPadding.bottom;
            ...
            LPCTSTR pstrText = NULL;
            if(pCallback) pstrText = pCallback->GetItemText(this, j, i);
            else pstrText = GetCellText(i, j);

            if(pInfo->bShowHtml)
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem, pstrText, iTextColor, \
                (m_pRcLinks + m_nLinks), (m_pSzLinks + m_nLinks), nLinks, DT_SINGLELINE | pInfo->uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rcItem, pstrText, iTextColor, \
                pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);

            m_nLinks    += nLinks;
            nLinks      = m_nMaxLinks - m_nLinks; 
        }
    }

    for(int i = m_nLinks; i < m_nMaxLinks; ++i)
    {
        ::ZeroMemory(m_pRcLinks + i, sizeof(RECT));
        (m_pSzLinks + i)->Empty();
    }
}

void CListGroupElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
    if(!m_pOwner) return;

    TListInfoUI* pInfo = m_pOwner->GetListInfo();
    DWORD iBackColor = 0;
    if(!pInfo->bAlternateBk || (m_iIndex & 0x00000001) == 0)
    {
        iBackColor = pInfo->dwBkColor;
    }
    if(m_bSelectable && (m_uButtonState & UISTATE_HOT) != 0)
    {
        iBackColor = pInfo->dwHotBkColor;
    }
    if(IsSelected())
    ...
    if(iBackColor)
    {
        RECT rcItem = {m_rcItem.left, m_rcItem.top /*+ 1*/, m_rcItem.right, m_rcItem.bottom - 1};
    ...
    if(pInfo->dwLineColor)
    {
        IListCallbackUI* pCallback = m_pOwner->GetTextCallback();
        DWORD dwAdjustColor = GetAdjustColor(pInfo->dwLineColor);
        for(int i = 0; i < m_nColumn; ++i)
        {
            for(int j = 0; j < m_pInnerRow[i]; ++j)
            {
                RECT rcRightLine  = {m_pCellRect[i][j].right - 1, m_pCellRect[i][j].top, m_pCellRect[i][j].right - 1, m_pCellRect[i][j].bottom};
                RECT rcBottomLine = {m_pCellRect[i][j].left, m_pCellRect[i][j].bottom - 1, m_pCellRect[i][j].right, m_pCellRect[i][j].bottom - 1};
                CRenderEngine::DrawLine(hDC, rcRightLine, 1, dwAdjustColor);
                CRenderEngine::DrawLine(hDC, rcBottomLine, 1, dwAdjustColor);
            }
        }
    }
}
...
CContainerItemUI::CContainerItemUI()
{

}

LPCTSTR CContainerItemUI::GetClass() const
{
	return _T("ContainerItem");
}

UINT CContainerItemUI::GetControlFlags() const
{
	return UIFLAG_WANTRETURN;
}

LPVOID CContainerItemUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcscmp(pstrName, _T("ContainerItem")) == 0 ) return static_cast<CContainerItemUI*>(this);
	return CContainerUI::GetInterface(pstrName);
}

SIZE CContainerItemUI::EstimateSize(SIZE szAvailable)
{
	if( m_cxyFixed.cy == 0 ) return CSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14);
	return CControlUI::EstimateSize(szAvailable);
}
