...
CControlUI* CControlUI::GetTabControl() const
{
	return m_pTabControl;
}

void CControlUI::SetTabControl(CControlUI* pControl)
{
	m_pTabControl = pControl;
}
...
void CControlUI::SetValue(LPCWSTR key, LPCWSTR value)
{
    m_mapUserValues[key] = value;
}

void CControlUI::SetValue(LPCWSTR key, int value)
{
    std::wstringstream stream;
    stream << value;

    m_mapUserValues[key] = stream.str().c_str();
}

void CControlUI::SetValue(LPCWSTR key, __int64 value)
{
    std::wstringstream stream;
    stream << value;
...
void CControlUI::SetValue(LPCWSTR key, double value)
{
    std::wstringstream stream;
    stream << value;
...
CStdString CControlUI::GetStringValue(LPCWSTR key)
{
    return m_mapUserValues[key];
}

int CControlUI::GetIntValue(LPCWSTR key)
{
    std::wstringstream stream;
    std::wstring str = m_mapUserValues[key];
    stream << str;
...
__int64 CControlUI::GetInt64Value(LPCWSTR key)
{
    std::wstringstream stream;
    std::wstring str = m_mapUserValues[key];
    stream << str;
...
double CControlUI::GetDoubleValue(LPCWSTR key)
{
    std::wstringstream stream;
    std::wstring str = m_mapUserValues[key];
    stream << str;
...
CControlUI* CControlUI::ApplyStyle(LPCTSTR pstrStyleName)
{
	if(m_pManager){
		LPCTSTR pStrAttrbutes = m_pManager->GetStyleAttributeList(pstrStyleName);
		if(pStrAttrbutes)
			this->ApplyAttributeList(pStrAttrbutes);
	}
	return this;
}
