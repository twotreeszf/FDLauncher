...
void CComboWnd::Init(CComboUI* pOwner)
{
	...
	int iMoveLeft = pOwner->GetDropboxMoveLeft();
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        m_pLayout->OnNotify += MakeDelegate(this, &CComboWnd::OnWindowInit);
        ...
        //m_pOwner->DoEvent(event);
        //EnsureVisible(m_pOwner->GetCurSel());
        if(event.wParam == SB_LINEDOWN)
        {
            SIZE sz = m_pLayout->GetScrollPos();
            m_pLayout->SetScrollPos(CSize(sz.cx, sz.cy + 25));
        }
        else
        {
            SIZE sz = m_pLayout->GetScrollPos();
...
bool CComboWnd::OnWindowInit(void* pArg)
{
    TNotifyUI* pNotify = static_cast<TNotifyUI*>(pArg);
    if(pNotify->sType == _T("windowinit"))
    {
        EnsureVisible(m_pOwner->GetCurSel());
    }
...
int CComboUI::GetSelCount() const
{
	if(m_iCurSel >= 0)
		return 1;
	return 0;
...
bool CComboUI::GetDropBoxWndPos(RECT rc )
{
	HWND hWnd = m_pWindow->GetHWND();
	return ::GetClientRect(hWnd, &rc);
}
void CComboUI::ModifyDropBoxSize(int deltLeft, int deltTop, int deltRight, int deltBottom)
{
	...
	if( deltLeft != -1 ) rc.left -= deltLeft;
	if( deltTop != -1 ) rc.top -= deltTop;
	if( deltRight != -1 ) rc.right += deltRight;
	if( deltBottom != -1 ) rc.bottom += deltBottom;
	if( !::AdjustWindowRectEx(&rc, GetWindowStyle(hWnd), (!(GetWindowStyle(hWnd) & WS_CHILD) && (::GetMenu(hWnd) != NULL)), GetWindowExStyle(hWnd)) ) return;
...
void CComboUI::SetDropboxMoveLeft(int iDropboxMoveLeft)
{
    m_iDropboxMoveLeft = iDropboxMoveLeft;
}

int CComboUI::GetDropboxMoveLeft() const
{
    return m_iDropboxMoveLeft;
}
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("dropboxmoveleft")) == 0) 
	{
		SetDropboxMoveLeft(_ttoi(pstrValue));
	}
