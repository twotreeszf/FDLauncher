...
extern unsigned long GetZipNumFiles(HZIP hz);
///////////////////////////////////////////////////////////////////////////////////////
...
CResourceUI::CResourceUI()
	...
	, m_hResourceZip(NULL)
{
	::InitializeCriticalSectionAndSpinCount(&m_csItemCache, 0);
}
...
CResourceUI::~CResourceUI()
{
	...
	for (int i = 0, len = m_zipItemCache.GetSize();i < len;i++)
	{
		LPCTSTR szKey = m_zipItemCache.GetAt(i);
		ZIPCACHEITEM* pItem = (ZIPCACHEITEM*)m_zipItemCache.Find(szKey);
		delete[] pItem->data;
		delete pItem;
	}

	while(m_zipItemCache.GetSize() > 0)
	{
		m_zipItemCache.Remove(m_zipItemCache.GetAt(0));
	}	

	::DeleteCriticalSection( &m_csItemCache );	
}

HINSTANCE CResourceUI::GetInstance()
{
	return m_hInstance;
}
...
const  CStdString& CResourceUI::GetResourcePath()
{
	return m_pStrResourcePath;
}

const  CStdString& CResourceUI::GetResourceZip()
{
...
void CResourceUI::SetInstance( HINSTANCE hInst )
{
	m_hInstance = hInst;
}
...
void CResourceUI::SetResourceZip(LPVOID pVoid, unsigned int len)
{
...
void CResourceUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip)
{
	...
	if( m_bCachedResourceZip ) {
		CStdString sFile = this->GetResourcePath();
		sFile += this->GetResourceZip();
		m_hResourceZip = (HANDLE)OpenZip((void*)sFile.GetData(), 0, 2);
	}
}

DWORD CResourceUI::FastFindZipItem(HANDLE hz, const TCHAR *name, bool ic, int *index, void *ze)
{
	CLock lock(&m_csItemCache);
	CStdString strKey(name);
	strKey.Replace(_T("\\"), _T("/"));
	strKey.MakeLower();
	LPVOID pData = m_zipItemCache.Find(strKey);
	if(pData != NULL)
	{
		ZIPCACHEITEM* pItem = (ZIPCACHEITEM*)pData;
		ZIPENTRY* pZe = (ZIPENTRY*)ze;
		*pZe = pItem->ze;
		return 0;
	}
	else
	{
		ZRESULT ret = ::FindZipItem((HZIP)hz, name, ic, index, (ZIPENTRY*)ze);
		if(ret != 0)return ret;
		ZIPCACHEITEM* pItem = new ZIPCACHEITEM;
		pItem->ze = *((ZIPENTRY*)ze);
		pItem->data = NULL;
		if(!m_zipItemCache.Insert(strKey, pItem))
		{
			delete pItem;
		}
		return ret;
...
DWORD CResourceUI::FastUnzipItem(HANDLE hz, const TCHAR *name, int index, void *dst, unsigned int len, DWORD flags)
{
	CLock lock(&m_csItemCache);
	CStdString strKey(name);
	strKey.Replace(_T("\\"), _T("/"));
	...
	if(pData != NULL)
	{
		...
		if(pItem->data == NULL)
		{
			ASSERT(len == pItem->ze.unc_size);
			pItem->data = new BYTE[pItem->ze.unc_size];
			ZRESULT res = ::UnzipItem((HZIP)hz, index, pItem->data, pItem->ze.unc_size, flags);
	...
	{
		ASSERT(FALSE);
		return ::UnzipItem((HZIP)hz, index, dst, len, flags);
	}
}

void  CResourceUI::RemoveZipCache(const TCHAR *name)
{
	CLock lock(&m_csItemCache);
	LPVOID pData = m_zipItemCache.Find(name);
	if(pData != NULL)
	{
		ZIPCACHEITEM* pItem = (ZIPCACHEITEM*)pData;
		if(pItem->data)
		...
		delete pItem;
		m_zipItemCache.Remove(name);
	}
}

void  CResourceUI::ExtractZipInMemory(long maxSize)
{
	CLock lock(&m_csItemCache);
	HZIP hz = NULL;
	...
	{
		CStdString sFile = GetResourcePath();
	...
	int index(0),count(0);
	ZIPENTRY ze;
	count = GetZipNumFiles(hz);
	while(index < count)
	{
		res = GetZipItem(hz, index, &ze);
		if(res == 0)
		{
			CStdString strKey(ze.name);
			strKey.Replace(_T("\\"), _T("/"));
			strKey.MakeLower();
			LPVOID pData = m_zipItemCache.Find(strKey);

			if(pData == NULL && ze.unc_size != 0 && (maxSize == -1 || ze.unc_size <= maxSize))
			{
				ZIPCACHEITEM* pItem = new ZIPCACHEITEM;		
				pItem->ze = ze;
				pItem->data = new BYTE[ze.unc_size];
				int ret = UnzipItem(hz, index, pItem->data, ze.unc_size, 3);
				if(ret != 0x00000000 && ret != 0x00000600)
				{
					delete[] pItem->data;
					delete pItem;					
				}
				...
				{					
					m_zipItemCache.Set(strKey, pItem);
				}
			}
		}
		index++;
	}
	if(!IsCachedResourceZip()) CloseZip(hz);
}
...
LPSHELLEXECUTEHOOK CPaintManagerUI::m_pShellExecuteHook = NULL;
...
void CPaintManagerUI::Init(HWND hWnd, CResourceUI* pRes)
{
...
CResourceUI* CPaintManagerUI::GetResource()
{
	return m_pResource;
}
...
HINSTANCE CPaintManagerUI::ShellExecuteX(__in_opt HWND hwnd, 
	...
	__in INT nShowCmd)
{
	if(m_pShellExecuteHook)
	{
		return m_pShellExecuteHook(hwnd, lpOperation, lpFile, lpParameters, lpDirectory, nShowCmd);
	}
	else
	{
...
void CPaintManagerUI::SetShellEcecuteHook(LPSHELLEXECUTEHOOK pHook)
{
	m_pShellExecuteHook = pHook;
}
    ...
    case WM_KEYDOWN:
        {
           ...
           if( wParam == VK_TAB ) {
			   ...
			   bool bNotShift = ::GetKeyState(VK_SHIFT) >= 0;
               CControlUI* pControl = m_pFocus ? m_pFocus->GetTabControl() : NULL;
			   if(pControl && pControl->IsVisible() && pControl->IsEnabled() && bNotShift)
			   {
				   SetFocus(pControl);
			   }
			   else
			   {
					SetNextTabControl(bNotShift);
			   }
               return true;
           }
        }
    ...
    case WM_APP + 1:
        {
			...
			{
				if(m_pFocus == m_aDelayedCleanup[i] ) m_pFocus = NULL;
				if(m_pEventClick == m_aDelayedCleanup[i]) m_pEventClick = NULL;
				if(m_pEventHover == m_aDelayedCleanup[i]) m_pEventHover = NULL;				
				if(m_pEventKey  == m_aDelayedCleanup[i])  m_pEventKey = NULL;
...
void CPaintManagerUI::AddDelayedCleanup(CControlUI* pControl, bool bPostMessage/* = true*/)
{
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( type != NULL ) {
        if( isdigit(*bitmap) ) {
            ...
            data = CRenderEngine::LoadImage(m_pResource, iIndex, type, mask);
        }
    }
    else {
        data = CRenderEngine::LoadImage(m_pResource, bitmap, NULL, mask);
    }
            ...
            if( data != NULL ) {
                if( !data->sResType.IsEmpty() ) {
                    if( isdigit(*bitmap) ) {
                        LPTSTR pstr = NULL;
                        int iIndex = _tcstol(bitmap, &pstr, 10);
                        pNewData = CRenderEngine::LoadImage(m_pResource, iIndex, data->sResType.GetData(), data->dwMask);
                    }
                }
                else {
                    pNewData = CRenderEngine::LoadImage(m_pResource, bitmap, NULL, data->dwMask);
                }
...
void CPaintManagerUI::AddStyleAttributeList(LPCTSTR pStrStyleName, LPCTSTR pStrStyleAttrList)
{
	CStdString* pDefaultAttr = new CStdString(pStrStyleAttrList);
	if (pDefaultAttr != NULL)
	{
		if (m_StyleAttrHash.Find(pStrStyleName) == NULL)
			m_StyleAttrHash.Set(pStrStyleName, (LPVOID)pDefaultAttr);
		else
			delete pDefaultAttr;
	}
}

LPCTSTR CPaintManagerUI::GetStyleAttributeList(LPCTSTR pStrStyleName) const
{
	CStdString* pDefaultAttr = static_cast<CStdString*>(m_StyleAttrHash.Find(pStrStyleName));
	if( !pDefaultAttr && m_pParentResourcePM ) return m_pParentResourcePM->GetStyleAttributeList(pStrStyleName);

	if( pDefaultAttr ) return pDefaultAttr->GetData();
	else return NULL;
}

bool CPaintManagerUI::RemoveStyleAttributeList(LPCTSTR pStrStyleName)
{
	CStdString* pDefaultAttr = static_cast<CStdString*>(m_StyleAttrHash.Find(pStrStyleName));
	if( !pDefaultAttr ) return false;

	delete pDefaultAttr;
	return m_StyleAttrHash.Remove(pStrStyleName);
}

const CStdStringPtrMap& CPaintManagerUI::GetStyleAttribultes() const
{
	return m_StyleAttrHash;
}
void CPaintManagerUI::RemoveAllStyleAttributeList()
{
	CStdString* pDefaultAttr;
	for( int i = 0; i< m_StyleAttrHash.GetSize(); i++ ) {
		if(LPCTSTR key = m_StyleAttrHash.GetAt(i)) {
			pDefaultAttr = static_cast<CStdString*>(m_StyleAttrHash.Find(key));
			delete pDefaultAttr;
		}
	}	
}
...
CControlUI* CPaintManagerUI::GetParentByClass(CControlUI* pSubControl, LPCTSTR pstrClass)
{
	CControlUI* pControl = pSubControl->GetParent();
	while(pControl != this->GetRoot())
	{
		if(_tcsicmp(pControl->GetClass(), pstrClass) == 0)
			return pControl;
