...
public:
	...
	virtual int  GetSelCount() const = 0;
...
{	
public:
    CListUI(CContainerUI* pListBody = NULL);
	...
	int  GetSelItemIndex(int i) const;
    ...
    void SetScrollLength(int iScrollLenth);
...
protected:
    bool SingleSelect(int iIndex, bool bTakeFocus, bool bEnsureVisible = true);
    bool MultiSelectItem(int iIndex, bool bSelect, bool bTakeFocus, bool bEnsureVisible = true);
    bool MultiSelectRange(int iIndex, bool bSelect, bool bTakeFocus);
private:
	void UnselectItem( int iIndex, bool bTakeFocus /*= false*/);
protected:
	...
	CStdPtrArray m_aCurSelItems; //多选支持
...
public:
	...
	CStdString GetToolTip() const;
	void SetToolTip(LPCTSTR pstrText);

	CStdString GetToolTip(int iIndex) const;
	void SetToolTip(int iIndex, LPCTSTR pstrText);
...
class UILIB_API CContainerItemUI : public CContainerUI
{
public:
	CContainerItemUI();
...
class UILIB_API CListGroupElementUI: public CControlUI, public IListItemUI
{
public:
    CListGroupElementUI(int nColumn, int pInnerRow[]);
    ~CListGroupElementUI();
    ...
    LPCTSTR GetCellText(int iColumnIndex, int iInnerRowIndex) const;
    void SetCellText(int iColumnIndex, int iInnerRowIndex, LPCTSTR pstrText);

    LPCTSTR GetToolTip(int iColumnIndex, int iInnerRowIndex) const;
    void SetToolTip(int iColumnIndex, int iInnerRowIndex, LPCTSTR pstrText);

    CStdString GetToolTip() const;
    void SetToolTip(LPCTSTR pstrText);

    void GetHoverCell(int& iHoverColumn, int& iHoverRow);
...
protected:
    const int       m_nColumn;
    int*            m_pInnerRow;
    CStdString**    m_pCellText;
    CStdString**    m_pToolTip;
    RECT**          m_pCellRect;
    int             m_iHoverColumn;
    int             m_iHoverRow;
