...
CLabelUI::CLabelUI() : m_uTextStyle(DT_VCENTER), m_dwTextColor(0), 
    m_dwDisabledTextColor(0), m_iFont(-1), m_bShowHtml(false), m_bMultiline(false)
{
...
void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( _tcscmp(pstrName, _T("align")) == 0 ) { 
		...
		if( _tcsstr(pstrValue, _T("editcontrol")) != NULL ){
			m_uTextStyle |= DT_EDITCONTROL;
		}
    }
...
void CLabelUI::PaintText(HDC hDC)
{
    ...
    if(IsMultiline())
    {
        uTextStyle |= (DT_WORDBREAK | DT_EDITCONTROL);
    }
    else 
    {
...
CButtonUI::CButtonUI() : m_uButtonState(0), m_dwHotTextColor(0), m_dwPushedTextColor(0), m_dwFocusedTextColor(0),m_iHotFont(-1)
...
void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("hotfont")) == 0 ) {
		LPTSTR pstr = NULL;
		m_iHotFont = _tcstol(pstrValue, &pstr, 10);
	}
...
void CButtonUI::PaintText(HDC hDC)
{
    ...
    if( m_bShowHtml )
	{
		UINT uTextStyle = (m_uTextStyle & ~DT_SINGLELINE);
...
void COptionUI::Selected(bool bSelected, bool SendMessage/* = true */)
{
...
LPCTSTR CIconButtonUI::GetNormalIconImage()
{
	return m_sNormalIconImage;
}

void CIconButtonUI::SetNormalIconImage(LPCTSTR pStrImage)
{
	m_sNormalIconImage = pStrImage;
	Invalidate();
}

LPCTSTR CIconButtonUI::GetHotIconImage()
{
	return m_sHotIconImage;
}

void CIconButtonUI::SetHotIconImage(LPCTSTR pStrImage)
{
	m_sHotIconImage = pStrImage;
	Invalidate();
}

LPCTSTR CIconButtonUI::GetDisableIconImage()
{
	return m_sDisableIconImage;
}

void CIconButtonUI::SetDisableIconImage(LPCTSTR pStrImage)
{
	m_sDisableIconImage = pStrImage;
	Invalidate();
}

void CIconButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	if( _tcscmp(pstrName, _T("normaliconimage")) == 0 )
		SetNormalIconImage(pstrValue);
	else if ( _tcscmp(pstrName, _T("hoticonimage")) == 0 )
		SetHotIconImage(pstrValue);
	else if ( _tcscmp(pstrName, _T("disableiconimage")) == 0 )
		SetDisableIconImage(pstrValue);
	else 
...
{
	if ( !m_sHotIconImage.IsEmpty()&&  (m_uButtonState & UISTATE_HOT) != 0 ) 
	{
		if ( !DrawImage(hDC, (LPCTSTR)m_sHotIconImage)) m_sHotIconImage.Empty();
	}
	else if ( !m_sDisableIconImage.IsEmpty()&&  (m_uButtonState & UISTATE_DISABLED) != 0 ) 
	{
		if ( !DrawImage(hDC, (LPCTSTR)m_sDisableIconImage)) m_sDisableIconImage.Empty();
	}
	else if( !m_sNormalIconImage.IsEmpty()) 
	{
		if( !DrawImage(hDC, (LPCTSTR)m_sNormalIconImage) ) m_sNormalIconImage.Empty();
	}
...
CTextUI::CTextUI() : m_nLinks(0), m_nHoverLink(-1), m_bHoverUnderline(false)
{
    m_uTextStyle = DT_WORDBREAK;
    m_rcTextPadding.left = 2;
    m_rcTextPadding.right = 2;
	m_nMaxLinks = MAX_LINK;
	m_rcLinks = new CRect[MAX_LINK];
	m_sLinks  = new CStdString[MAX_LINK];    
}
...
{
    if (_tcscmp(pstrName, _T("hoverunderline")) == 0) SetHoverUnderline(_tcscmp(pstrValue, _T("true")) == 0);
    else CLabelUI::SetAttribute(pstrName, pstrValue);
}
    ...
    if( event.Type == UIEVENT_BUTTONUP && IsEnabled() ) {
        for( int i = 0; i < m_nLinks; i++ ) {
            if( ::PtInRect(&m_rcLinks[i], event.ptMouse) ) {
				...
				CStdString* pStr = this->GetLinkContent(i);
				if(pStr)
				{
					if(pStr->Find(_T("http://")) == 0)
					{
						CPaintManagerUI::ShellExecuteX(m_pManager->GetPaintWindow(), L"open", *pStr,NULL, NULL, SW_SHOWNORMAL);
					}
					else if(pStr->Find(_T("http:")) == 0)
					{
						CStdString strUrl = _T("http://");
						strUrl.Append(pStr->Mid(5));
						CPaintManagerUI::ShellExecuteX(m_pManager->GetPaintWindow(), L"open", strUrl, NULL, NULL, SW_SHOWNORMAL);
					}					
				}
...
void CTextUI::PaintText(HDC hDC)
{
    ...
    rc.bottom -= m_rcTextPadding.bottom;
    CStdString sText;
    if (m_bShowHtml && m_bHoverUnderline&& m_nHoverLink != -1)
    {
        sText = L"{u}";
        sText += m_sText + L"{\u}";
    }
...
void CTextUI::SetMaxLink(int nCount)
{
	if(m_nMaxLinks < nCount)
	{
		delete [] m_rcLinks;
		delete [] m_sLinks;

		m_nMaxLinks = nCount;
		m_rcLinks = new CRect[m_nMaxLinks];
		m_sLinks  = new CStdString[m_nMaxLinks];
	}
}

void CTextUI::SetHoverUnderline(bool bHoverUnderline)
{
    if (m_bHoverUnderline == bHoverUnderline) return;
    m_bHoverUnderline = bHoverUnderline;
    Invalidate();
}
...
CProgressImageUI::CProgressImageUI()
{
    m_nPart = 0;
    m_nAdd = 0;
    m_sPartForeImage.Empty();
    m_sFullForeImage.Empty();
}

LPCTSTR CProgressImageUI::GetClass() const
{
    return _T("ProgressImageUI");
}

LPVOID CProgressImageUI::GetInterface(LPCTSTR pstrName)
{
    if (_tcscmp(pstrName, _T("ProgressImage")) == 0) return static_cast<CProgressImageUI*>(this);
    return CProgressUI::GetInterface(pstrName);
}

void CProgressImageUI::SetPartForeImage(LPCTSTR pStrImage)
{
    if( m_sPartForeImage == pStrImage ) return;

    m_sPartForeImage = pStrImage;
    Invalidate();
}

void CProgressImageUI::SetFullForeImage(LPCTSTR pStrImage)
{
    if( m_sFullForeImage == pStrImage ) return;

    m_sFullForeImage = pStrImage;
    Invalidate();
}

void CProgressImageUI::SetPartValue(int nPart)
{
    m_nPart = nPart;
    Invalidate();
}

void CProgressImageUI::SetAddValue(int nAdd)
{
    m_nAdd = nAdd;
    Invalidate();
}

void CProgressImageUI::SetValue(int nValue)
{
    m_nValue = (nValue + m_nAdd) * (m_nMax - m_nMin) / (m_nMax - m_nMin + m_nAdd);
    Invalidate();
}

void CProgressImageUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( _tcscmp(pstrName, _T("partforeimage")) == 0 ) SetPartForeImage(pstrValue);
    else if( _tcscmp(pstrName, _T("fullforeimage")) == 0 ) SetFullForeImage(pstrValue);
    else if( _tcscmp(pstrName, _T("part")) == 0 ) SetPartValue(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, _T("add")) == 0 ) SetAddValue(_ttoi(pstrValue));
    else CProgressUI::SetAttribute(pstrName, pstrValue);
}

void CProgressImageUI::PaintStatusImage(HDC hDC)
{
    if(m_nValue > m_nPart && !m_sFullForeImage.IsEmpty())
    {
        m_sForeImage = m_sFullForeImage;
    }
    else if(!m_sPartForeImage.IsEmpty())
    {
        m_sForeImage = m_sPartForeImage;
    }
...
void CEditWnd::Init(CEditUI* pOwner)
{
    ...
    if( m_pOwner->IsMultiline()) uStyle |= ES_MULTILINE | ES_WANTRETURN;
...
LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_KILLFOCUS ) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
        ...
        m_pOwner->GetManager()->SendNotify(m_pOwner, _T("esc"));
    }
    else if (uMsg == WM_KEYDOWN && TCHAR(wParam) == VK_F5) {
        m_pOwner->GetManager()->SendNotify(m_pOwner, _T("F5"));
    }
...
m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0), m_nStartChar(0), m_nEndChar(-1)
{
...
void CEditUI::DoEvent(TEventUI& event)
{
    ...
    {
		if(m_pWindow != NULL && ::GetFocus() != m_pWindow->GetHWND())
			SendMessage(m_pWindow->GetHWND(), WM_KILLFOCUS, 0, 0);
        Invalidate();
    }
...
void CEditUI::SetPromptText(LPCTSTR pStrText)
{
	m_sPromptText = pStrText;
}

LPCTSTR CEditUI::GetPromptText()const
{
	return m_sPromptText;
}

void CEditUI::PaintSel()
{
    if( m_pWindow != NULL ) Edit_SetSel(*m_pWindow, m_nStartChar, m_nEndChar);
}
...
m_uButton2State(0), m_uThumbState(0), m_nThumbMinSize(0), m_bShowButton1(true), m_bShowButton2(true)
{
    ...
    if( m_bHorizontal ) {
        ...
        if( cx > m_cxyFixed.cy ) {
            ...
            if( m_nRange > 0 ) {
				...
				if( m_nThumbMinSize > 0 && cxThumb < m_nThumbMinSize) cxThumb = m_nThumbMinSize;
        ...
        if( cy > m_cxyFixed.cx ) {
            ...
            if( m_nRange > 0 ) {
				...
				if( m_nThumbMinSize > 0 && cyThumb < m_nThumbMinSize) cyThumb = m_nThumbMinSize;
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_MOUSEMOVE )
    {
        if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
            if( !m_bHorizontal ) {				
				int nButtonHeight = (m_rcButton1.bottom - m_rcButton1.top) + (m_rcButton2.bottom - m_rcButton2.top);
                int x = m_rcItem.bottom - m_rcItem.top - m_rcThumb.bottom + m_rcThumb.top - nButtonHeight/*2 * m_cxyFixed.cx*/;
                m_nLastScrollOffset = ((x == 0) ? 0 : ((event.ptMouse.y - ptLastMouse.y) * m_nRange / x ));
            }
            else {
                int nButtonWidth = (m_rcButton1.right - m_rcButton1.left) + (m_rcButton2.right - m_rcButton2.left);
                int x = m_rcItem.right - m_rcItem.left - m_rcThumb.right + m_rcThumb.left - nButtonWidth/*2 * m_cxyFixed.cy*/;
                m_nLastScrollOffset = ((x == 0) ? 0 : ((event.ptMouse.x - ptLastMouse.x) * m_nRange / x));
            }
        }
        else {
...
void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("thumbminsize")) == 0) SetThumbMinSize(_ttoi(pstrValue));
    else CControlUI::SetAttribute(pstrName, pstrValue);
}
...
HICON CFileIconUI::GetIcon()
{
    return m_hIcon;
}

void CFileIconUI::SetIcon(HICON hIcon)
{
    if (m_hIcon == hIcon)
    {
        return;
...
LPCTSTR CFileIconUI::GetNormalImage()
{
    return m_sNormalImage;
}

void CFileIconUI::SetNormalImage(LPCTSTR pStrImage)
{
    m_sNormalImage = pStrImage;
    Invalidate();
}

void CFileIconUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if (_tcscmp(pstrName, _T("normalimage")) == 0) SetNormalImage(pstrValue);
    else CControlUI::SetAttribute(pstrName, pstrValue);
}

void CFileIconUI::PaintBkImage(HDC hDC)
{
    ...
    if (m_hIcon)
    {
        ::DrawIconEx(hDC, m_rcItem.left + m_rcPadding.left, m_rcItem.top + m_rcPadding.top, m_hIcon,
            (m_cxyFixed.cx), (m_cxyFixed.cy), 0, NULL, DI_NORMAL | DI_COMPAT);
    }
