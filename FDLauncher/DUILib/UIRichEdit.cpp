...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
	...
	if (!hmod) hmod = LoadLibrary(_T("Riched20.dll"));
	if (!hmod) hmod = LoadLibrary(_T("Riched32.dll"));
	if (hmod) TextServicesProc = (PCreateTextServices)GetProcAddress(hmod, "CreateTextServices");
