...
void CContainerUI::RemoveAll()
{
    for( int it = 0; m_bAutoDestroy && it < m_items.GetSize(); it++ ) {
        if( m_bDelayedDestroy && m_pManager ) m_pManager->AddDelayedCleanup(static_cast<CControlUI*>(m_items[it]), false);             
        else delete static_cast<CControlUI*>(m_items[it]);
    }
    if (m_bDelayedDestroy)
    {
...
int CContainerUI::GetScrollLenth() const
{
    return m_iScrollLenth;
}
...
void CContainerUI::SetScrollLenth(int iScrollLenth)
{
    m_iScrollLenth = iScrollLenth;
    NeedUpdate();
}
...
{
    int cyLine = m_iScrollLenth;
    if (m_pManager) cyLine = m_pManager->GetDefaultFontInfo()->tm.tmHeight + m_iScrollLenth;
...
void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcscmp(pstrName, _T("scrolllength")) == 0) SetScrollLenth(_ttoi(pstrValue)); 
...
void CVerticalLayoutUI::EnsureVisible(int iIndex)
{
...
void CVerticalLayoutUI::Scroll(int dx, int dy)
{
    if (dx == 0 && dy == 0) return;
    SIZE sz = GetScrollPos();
...
{
    return m_nChildVPadding;
}

void CTileLayoutUI::SetChildVPadding(int nChildVPadding)
{
    m_nChildVPadding = nChildVPadding;
    NeedUpdate();
}

int CTileLayoutUI::GetChildHPadding() const
{
    return m_nChildHPadding;
}

void CTileLayoutUI::SetChildHPadding(int nChildHPadding)
{
    m_nChildHPadding = nChildHPadding;
    NeedUpdate();
}
...
void CTileLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("childhpadding")) == 0 ) SetChildHPadding(_ttoi(pstrValue));
    else CContainerUI::SetAttribute(pstrName, pstrValue);
}
...
void CTileLayoutUI::SetPos(RECT rc)
{
    ...
    // Position the elements
    if( m_szItem.cx > 0 ) m_nColumns = (rc.right - rc.left) / (m_szItem.cx + m_nChildHPadding);
    ...
    for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
        ...
        if( (iCount % m_nColumns) == 0 )
        {
            ...
            for( int it2 = it1; it2 < m_items.GetSize(); it2++ ) {
                ...
                if( iIndex == iCount || (iIndex + 1) % m_nColumns == 0 ) {
                    szAvailable.cx -= m_nChildHPadding / 2;
                }
                else {
                    szAvailable.cx -= m_nChildHPadding;
                }
        ...
        rcTile.left += rcPadding.left + m_nChildHPadding / 2;
        rcTile.right -= rcPadding.right + m_nChildHPadding / 2;
        if( (iCount % m_nColumns) == 0 ) {
            rcTile.left -= m_nChildHPadding / 2;
        }

        if( ( (iCount + 1) % m_nColumns) == 0 ) {
            rcTile.right += m_nChildHPadding / 2;
        }
        ...
        if( (++iCount % m_nColumns) == 0 ) {
            ptTile.x = iPosX;
            ptTile.y += cyHeight + m_nChildVPadding;
            cyHeight = 0;
        }
        else {
            ptTile.x += cxWidth;
...
int CTabLayoutUI::SelectedIndex()
{
    return m_iCurSel;
}
...
void CChildWindowUI::Init()
{
	if (!m_pstrXMLFile.IsEmpty())
	{
		...
		CContainerUI* pChildWindow = static_cast<CContainerUI*>(builder.Create(m_pManager->GetResource(), m_pstrXMLFile.GetData(), (UINT)0));
		if (pChildWindow)
		{
